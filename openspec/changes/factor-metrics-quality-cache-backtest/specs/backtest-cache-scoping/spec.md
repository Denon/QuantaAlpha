...
The system SHALL generate backtest cache keys as an MD5 hash of the tuple `(expression, market_identifier, date_start, date_end, provider_uri)`.
...
The system SHALL validate that cached factor data contains instruments matching the configured market universe before returning a cache hit.

#### Scenario: Cache instruments match configured market
- **WHEN** a cached factor file's instrument set is a subset of (or identical to) the configured market's instrument set
- **THEN** the cache hit SHALL be accepted

#### Scenario: Cache instruments do not match configured market
- **WHEN** a cached factor file's instrument set has less than 80% overlap with the configured market's instrument set
...
- **THEN** the new result SHALL be cached with a scoped key
