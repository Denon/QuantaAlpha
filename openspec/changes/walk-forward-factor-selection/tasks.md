...
- [ ] 1.1 Write failing tests for `generate_walk_forward_folds` with half-year windows, selection lag boundaries, and edge cases (tests/test_walk_forward.py)
- [ ] 1.2 Implement `WalkForwardConfig`, `WalkForwardFold`, `generate_walk_forward_folds`, and `load_walk_forward_config` in `quantaalpha/backtest/walk_forward.py`
- [ ] 1.3 Run fold generation tests and verify PASS
- [ ] 1.4 Commit: `feat: add walk-forward fold generation and config loading`

## 2. Factor Selection Scoring

- [ ] 2.1 Write failing tests for `select_top_factors` verifying window slicing, top-k ranking, and NaN rejection (tests/test_factor_selection.py)
- [ ] 2.2 Implement `FactorScore`, `FactorSelectionResult`, `score_factor_window`, and `select_top_factors` in `quantaalpha/backtest/factor_selection.py` using abs(Rank_IC) > abs(Rank_ICIR) > n_days as sort key
- [ ] 2.3 Run factor selection tests and verify PASS
- [ ] 2.4 Commit: `feat: add rolling factor selection scoring`

## 3. Precomputed Dataset Builder

- [ ] 3.1 Write failing test for `build_precomputed_dataset` verifying feature/label alignment and segment assignment (tests/test_walk_forward.py)
- [ ] 3.2 Create `quantaalpha/backtest/precomputed_dataset.py` with index normalization, NaN/inf handling, cross-sectional rank normalization, and Qlib `DatasetH` construction
- [ ] 3.3 Refactor `BacktestRunner._create_dataset_with_computed_factors` to delegate to `build_precomputed_dataset`
- [ ] 3.4 Run dataset and metrics tests and verify PASS
- [ ] 3.5 Commit: `refactor: extract precomputed dataset builder`
...
- [ ] 6.6 Commit: `feat: expose walk-forward backtest mode`
